'''
The time complexity of a few functionalities can be improved if we add a tail
pointer as well to the linked list.

Space and Time complexities for a Singly linked list WITH tail:
    1. Space complexity: O(n)
    2. Get element at start: O(1)
    3. Inserting an element at start: O(1)
    4. Deleting an element at start: O(1)
    5. Get element at end: O(1) # Was O(n) without tail
    6. Inserting an element at end: O(1) # Was O(n) without tail
...
class SLinkedListWithTail:
    ...
    '''
    Class for Singly linked list with tail
    '''
    ...
    def delete_last_node(self):  # Same as for without tail
